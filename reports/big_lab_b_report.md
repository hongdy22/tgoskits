...
## 七、StarryOS 架构设计分析与改进思考

StarryOS 的整体架构图和基本功能模块划分见 `./big_lab_b_PPT.pdf` 中 StarryOS 架构与模块介绍的相应图片。这里不再重复画图，而是结合本次实验中实际修复 syscall、BusyBox 和 Codex CLI 兼容问题的经历，对当前架构的优点、不足和可能的改进方向做一次总结。

### 7.1 当前架构的主要优点

第一，StarryOS 建立在 ArceOS 组件之上，能够复用 ArceOS 已经提供的调度、文件系统、网络栈、设备抽象等基础能力。这样做的优势是开发重心可以放在 Linux 兼容层，而不是从零实现一个完整内核。对于 Big Lab B 这种以应用兼容性为目标的实验来说，这种复用非常重要：内核可以较快具备运行真实程序所需的基础环境，然后通过兼容层不断补齐 Linux ABI。

第二，当前 StarryOS 的模块边界比较直观。内核代码大体按 `syscall`、`task`、`mm`、`file`、`net` 等模块组织，基本对应操作系统的核心组成部分。这样的结构降低了定位问题的成本。例如 syscall 参数校验问题通常先看 `syscall` 层，进程生命周期问题看 `task`，路径、fd、procfs 问题看 `file`，route netlink、packet socket、TCP option 等问题看 `net`。在本次实验中，Codex pipeline 能够稳定地从失败现象追到具体模块，和这种清晰划分有直接关系。

第三，StarryOS 的目标不是只通过若干人工 syscall 单元测试，而是面向真实 Linux 应用兼容。BusyBox、curl、git、Codex CLI 这类程序会把多个 ABI 面同时组合起来使用，能暴露单点测试很难发现的问题。例如 `busybox ip addr` 暴露 route netlink 格式问题，`busybox pidof` 暴露 `/proc/<pid>` 和用户可见 PID 问题，Codex CLI 又进一步暴露 `epoll` 用户缓冲区、`getcwd` raw syscall、zombie 生命周期和 tmpfs cleanup 等问题。这种“真实程序反向测试内核”的方式，比只验证简单 syscall 返回值更接近 Linux 兼容性的本质。

第四，统一的 fd/FileLike 抽象是一个很好的方向。普通文件、目录、socket、pipe、epoll 等对象都可以放进 fd 表，通过统一接口暴露 `read/write/poll/ioctl/stat` 等能力。这符合 Linux“一切皆文件”的接口风格，也让不同内核对象可以共享 fd 生命周期、非阻塞标志、poll/epoll 等机制。Exp3 中的 netlink、packet socket 和 Exp4 中的 epoll 问题都说明，统一 fd 抽象是支撑真实应用的重要基础。

### 7.2 当前架构的不足

当前最大的问题是，Linux 兼容功能仍然偏“按失败点逐步补齐”，还没有沉淀成统一的 ABI/状态治理层。StarryOS 已经能够通过很多真实应用测试，但不少兼容能力仍是“哪个应用失败就补哪个路径”。这种方式短期很有效，但长期会导致 ABI 规则、系统状态和生命周期约束分散在各个 syscall 或接口实现里。

一个典型例子是 ABI 错误语义分散。在 `linkat`、`renameat2` 这类 syscall 中，非法 flags 和坏用户指针同时出现时，Linux 对错误优先级有明确行为。如果每个 syscall 自己决定先读取用户指针还是先校验 flags，就容易和 Linux 不一致。类似问题不会只影响单个接口，而会让兼容层整体缺少一致的错误处理纪律。

因此，当前架构的不足不只是“还缺哪些 syscall”，而是缺少一个集中治理 Linux ABI 的层次。后续应该把 StarryOS 从“按应用补兼容路径”推进到“统一架构治理”：在 syscall 层形成清晰的 ABI wrapper，集中处理参数校验、用户指针和 errno；进程、网络、文件系统等模块也应沉淀类似的状态规则，并用文档或回归测试固定跨模块不变量。

### 7.3 如果重新设计或继续改进

如果重新设计，我会把当前隐含的 Linux 兼容路径显式拆成几层：

```text
Linux 应用
  -> Linux ABI wrapper 层：参数、用户指针、errno、错误优先级
  -> StarryOS 兼容语义层：进程、文件、网络、内存等用户可见状态
  -> ArceOS 基础能力层
  -> 平台与硬件
```

核心思路是把 `syscall` 层从简单分发器推进为 ABI wrapper，集中处理参数校验、用户指针访问、Linux errno 映射和错误优先级；同时让进程、网络、文件系统等模块维护更统一的用户可见状态，避免同一状态在多个接口中各自合成。

工程上可以继续保持“真实应用驱动”，但每个修复最好沉淀成一个明确的 ABI contract：说明 Linux 行为、StarryOS 差异、修复后的不变量和回归测试。这样 StarryOS 的兼容性改进就不会只是不断补洞，而会逐步形成可维护、可解释的 Linux 兼容架构。
...
### 8.3 Pipeline 的关键不是自动写代码，而是自动维持纪律
